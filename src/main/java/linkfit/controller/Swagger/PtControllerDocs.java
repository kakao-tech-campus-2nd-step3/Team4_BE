...
public interface PtControllerDocs {
    ...
    @ApiResponses({
        ...
        @ApiResponse(responseCode = "401", description = "인증 필요")})
    ResponseEntity<Void> sendSuggestion(@Parameter(hidden = true) @LoginTrainer Long trainerId,
    ...
    ResponseEntity<Void> approvalSuggestion(
        @Parameter(hidden = true) @LoginUser Long userId, @PathVariable Long ptId);
    ...
    ResponseEntity<Void> recallSuggestion(
        @Parameter(hidden = true) @LoginTrainer Long trainerId, @PathVariable Long ptId);
